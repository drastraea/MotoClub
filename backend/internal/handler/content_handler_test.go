...
// publicOnly is restricted to public rows for anonymous callers and visitors;
// members and up see everything.
func TestListPublicOnlyByRole(t *testing.T) {
	cases := []struct {
		name       string
		...
		{"anonymous", nil, true},
		{"visitor", &domain.Principal{Role: domain.RoleVisitor}, true},
		{"member", &domain.Principal{Role: domain.RoleMember}, false},
		{"admin", &domain.Principal{Role: domain.RoleAdmin}, false},
		{"superadmin", &domain.Principal{Role: domain.RoleSuperadmin}, false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			svc := svcmocks.NewMockEventServicer(t)
			svc.On("List", mock.Anything, mock.Anything, tc.publicOnly).Return([]domain.Event{}, nil)
			c, w := ctxJSON(http.MethodGet, "/events", "")
			if tc.principal != nil {
				httpx.SetPrincipal(c, *tc.principal)
			}
...
func TestEventList(t *testing.T) {
	...
	t.Run("service error", func(t *testing.T) {
		svc := svcmocks.NewMockEventServicer(t)
		svc.On("List", mock.Anything, mock.Anything, mock.Anything).Return(nil, apperr.ErrNotFound)
		c, w := ctxJSON(http.MethodGet, "/events", "")
		NewEventHandler(svc).List(c)
	...
	t.Run("success with filter", func(t *testing.T) {
		svc := svcmocks.NewMockEventServicer(t)
		svc.On("List", mock.Anything, mock.Anything, mock.Anything).Return([]domain.Event{{ID: 1, Title: "t", IsPublic: true}}, nil)
		c, w := ctxJSON(http.MethodGet, "/events?startFrom=2026-01-01", "")
		NewEventHandler(svc).List(c)
		assert.Equal(t, http.StatusOK, w.Code)
		assert.Contains(t, w.Body.String(), `"id":"1"`)
		assert.Contains(t, w.Body.String(), `"is_public":true`)
	})
}
...
func TestEventGet(t *testing.T) {
	...
	t.Run("not found", func(t *testing.T) {
		svc := svcmocks.NewMockEventServicer(t)
		svc.On("Get", mock.Anything, int64(1), mock.Anything).Return(domain.Event{}, apperr.ErrNotFound)
	...
	t.Run("success", func(t *testing.T) {
		svc := svcmocks.NewMockEventServicer(t)
		svc.On("Get", mock.Anything, int64(1), mock.Anything).Return(domain.Event{ID: 1, Title: "t", Description: "d"}, nil)
		c, w := ctxJSON(http.MethodGet, "/event/1", "")
		setParam(c, "id", "1")
		NewEventHandler(svc).Get(c)
		assert.Equal(t, http.StatusOK, w.Code)
...
func TestAnnouncementList(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		svc := svcmocks.NewMockAnnouncementServicer(t)
		svc.On("List", mock.Anything, mock.Anything, mock.Anything).Return([]domain.Announcement{{ID: 1, Title: "t"}}, nil)
		c, w := ctxJSON(http.MethodGet, "/announcements", "")
		NewAnnouncementHandler(svc).List(c)
		assert.Equal(t, http.StatusOK, w.Code)
...
func TestAnnouncementCreate(t *testing.T) {
	...
	t.Run("service error", func(t *testing.T) {
		svc := svcmocks.NewMockAnnouncementServicer(t)
		svc.On("Create", mock.Anything, "t", "d", false).Return(domain.Announcement{}, apperr.ErrValidation)
	...
	t.Run("success", func(t *testing.T) {
		svc := svcmocks.NewMockAnnouncementServicer(t)
		svc.On("Create", mock.Anything, "t", "d", false).Return(domain.Announcement{ID: 5}, nil)
		c, w := ctxJSON(http.MethodPost, "/announcements", `{"title":"t","description":"d"}`)
		NewAnnouncementHandler(svc).Create(c)
		assert.Equal(t, http.StatusCreated, w.Code)
...
func TestAnnouncementUpdate(t *testing.T) {
	...
	t.Run("service error", func(t *testing.T) {
		svc := svcmocks.NewMockAnnouncementServicer(t)
		svc.On("Update", mock.Anything, int64(1), "t", "d", false).Return(domain.Announcement{}, apperr.ErrNotFound)
	...
	t.Run("success", func(t *testing.T) {
		svc := svcmocks.NewMockAnnouncementServicer(t)
		svc.On("Update", mock.Anything, int64(1), "t", "d", false).Return(domain.Announcement{ID: 1}, nil)
...
func TestGallery(t *testing.T) {
	...
	t.Run("list success", func(t *testing.T) {
		svc := svcmocks.NewMockGalleryServicer(t)
		svc.On("List", mock.Anything, mock.Anything).Return([]domain.GalleryItem{{ID: 1, Link: "http://x", IsPublic: true}}, nil)
		c, w := ctxJSON(http.MethodGet, "/gallery", "")
		NewGalleryHandler(svc).List(c)
		assert.Equal(t, http.StatusOK, w.Code)
	...
	t.Run("create service error", func(t *testing.T) {
		svc := svcmocks.NewMockGalleryServicer(t)
		svc.On("Create", mock.Anything, "http://x", false).Return(domain.GalleryItem{}, apperr.ErrValidation)
		c, w := ctxJSON(http.MethodPost, "/gallery", `{"link":"http://x"}`)
		NewGalleryHandler(svc).Create(c)
		assert.Equal(t, http.StatusBadRequest, w.Code)
	})
	t.Run("create success with is_public", func(t *testing.T) {
		svc := svcmocks.NewMockGalleryServicer(t)
		svc.On("Create", mock.Anything, "http://x", true).Return(domain.GalleryItem{ID: 8}, nil)
		c, w := ctxJSON(http.MethodPost, "/gallery", `{"link":"http://x","is_public":true}`)
		NewGalleryHandler(svc).Create(c)
		assert.Equal(t, http.StatusCreated, w.Code)
		assert.JSONEq(t, `{"id":"8"}`, w.Body.String())
	})
	t.Run("update bad id", func(t *testing.T) {
		c, w := ctxJSON(http.MethodPut, "/gallery/x", `{"is_public":true}`)
		setParam(c, "id", "x")
		NewGalleryHandler(svcmocks.NewMockGalleryServicer(t)).Update(c)
		assert.Equal(t, http.StatusBadRequest, w.Code)
	})
	t.Run("update service error", func(t *testing.T) {
		svc := svcmocks.NewMockGalleryServicer(t)
		svc.On("Update", mock.Anything, int64(1), true).Return(domain.GalleryItem{}, apperr.ErrNotFound)
		c, w := ctxJSON(http.MethodPut, "/gallery/1", `{"is_public":true}`)
		setParam(c, "id", "1")
		NewGalleryHandler(svc).Update(c)
		assert.Equal(t, http.StatusNotFound, w.Code)
	})
	t.Run("update success", func(t *testing.T) {
		svc := svcmocks.NewMockGalleryServicer(t)
		svc.On("Update", mock.Anything, int64(1), true).Return(domain.GalleryItem{ID: 1}, nil)
		c, _ := ctxJSON(http.MethodPut, "/gallery/1", `{"is_public":true}`)
		setParam(c, "id", "1")
		NewGalleryHandler(svc).Update(c)
		assert.Equal(t, http.StatusNoContent, c.Writer.Status())
	})
